...
Randomized controlled trials (RCTs) are the gold standard for causal inference. Because treatment assignment is random, we can attribute differences in outcomes to the treatment rather than to confounders. In practice, treatment effects are typically estimated with $t$-tests or regression adjustments. With binary outcomes a prudent data scientist might naturally be drawn to fit a logistic regression instead. But how does logistic regression fare in randomized trials?

In this post, we'll walk through the statistical intuition based on David Freedman's well-known 2008 paper, and discuss what practitioners should do instead. We'll explore when logistic regression can be misleading in randomized trials—and what you can do to about it.
...
Each unit has two potential outcomes: $Y_i^T$ (if treated) and $Y_i^C$ (if control). Only one of these is observed, depending on $X_i$.
...
This is the difference in the log-odds of success between treatment and control. You might remember from your econometrics course that in logistic regression, it is indeed these log-odds that are modeled to be linear in the right-hand side variables. 

This is the estimand of interest in our setting.
