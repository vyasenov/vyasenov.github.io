...
Balance is thus fundamental to causal inference. [Weighting methods](https://doi.org/10.1080/01621459.2016.1260466), central to achieving this balance, have evolved to include two primary types: inverse propensity score weights and covariate balancing weights. (These are distinct from the [general types of weights](https://vyasenov.github.io/blog/weights-statistics.html) one encounters in statistical analyses more broadly.) This article briefly describes these weights, their mathematical foundations, and the intuition behind them, with an example in `R` to bring it all together.
...
A second key property is that the weighted average of the outcomes for the treated group is an unbiased estimator for the mean outcome under treatment, $\mu_1$. This is a cleaner target than the implicit weighting baked into a regression, which I have discussed in [what OLS estimates in causal inference](https://vyasenov.github.io/blog/interpret-OLS-causal-inference.html). This can be expressed mathematically as:
...
Third, these IPS weights are generally unknown and have to be estimated from the data. A leading exception is controlled randomized experiments where the researcher determines the probability of treatment. This is, however, uncommon. Traditionally, practitioners rely on methods like logistic regression to first obtain $\hat{e}(X)$ and then take its reciprocal to construct the weights. Weighting is, of course, just one of [many flavors of propensity score methods](https://vyasenov.github.io/blog/flavors-prop-score-methods.html).
...
An alternative approach seeks weights that directly balance the dataset at hand. Typically this is framed as a constrained optimization problem placing restrictions on the maximum imbalance in $X$ between the treatment and control groups.
...
with $f(x_i)=x$. The same idea can be applied to higher moments of $X$ like variance or skewness. Example methods relying on this type of weights include [Hainmueller (2012)](https://doi.org/10.1093/pan/mpr025), [Chan et al. (2016)](https://doi.org/10.1111/rssb.12129), and [Athey et al. (2018)](https://doi.org/10.1111/rssb.12268), among many others.

Covariate balancing weights bypass the need to explicitly estimate $e(X)$. Instead, they solve for weights that ensure the treated and control groups are balanced across predefined covariates or their transformations. This method aligns closely with the goal of causal inference: achieving balance. The main challenge is selecting the right set of covariates to be balanced. [Variance estimation](https://vyasenov.github.io/blog/variance-ps-matching.html) can also be more involved, although modern statistical packages take care of it.
...
- Covariate balancing weights directly target balance in $X$, bypassing propensity score estimation.
