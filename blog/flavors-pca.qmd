...
In this post I will use the standard PCA formulation as the baseline and then focus on four variants that I think matter most in applied work: Sparse PCA for interpretability, Kernel PCA for nonlinear structure, Probabilistic PCA for likelihood-based modeling and missing data, and Truncated or Randomized PCA for large-scale computation. I am leaving out several other branches, including robust PCA and functional PCA, not because they are unimportant, but because they are better treated as separate topics.
...
- Truncated or randomized PCA is often the most practical choice on large matrices because it targets the same principal subspace at a much lower computational cost.
