...
But here's the thing: there isn’t just *one* bootstrap. Over the years, statisticians have developed many flavors of the bootstrap to address different challenges in different settings. Some handle small samples better. Some are designed for dependent data like time series. Others shine when the [assumptions of classic bootstrapping crumble](https://vyasenov.github.io/blog/bootstrap-limitations.html) (think clustered data or heteroskedasticity).
...
Let’s start with the jackknife, developed back in the 1950s by Quenouille and popularized by Tukey. The [jackknife isn’t technically a bootstrap](https://vyasenov.github.io/blog/jackknife-vs-bootstrap.html), but it’s often the gateway to resampling methods. Here is how it works:
...
1. Sample clusters $g$ from your data with replacement and include *all observations* $i$ from each selected cluster.
2. Compute the statistic $\hat{\theta}^*_b = T(\hat{F}^*_b)$.
:::

**Strengths:** Simple to implement, preserves cluster dependence, consistent under many forms of within-cluster correlation.

**Weaknesses:** Requires a reasonably large number of clusters (typically $G \geq 30$). Can be biased or unstable with few clusters. (Luckily, the United States was broken down into 50 states. The Swiss were not as fortunate.)
...
- Specialized bootstraps (wild, block, Bayesian, subsampling) handle heteroskedasticity, clustering, dependence, and other real-world challenges that trip up the classic approach. I have even written about a more exotic [column-sampling variant](https://vyasenov.github.io/blog/column-sampling-bootstrap.html) for wide datasets.
