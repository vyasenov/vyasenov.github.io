...
Lord’s paradox presents a fascinating challenge in causal inference and statistics. It highlights how different statistical methods applied to the same data can lead to contradictory conclusions. The paradox typically arises when comparing changes over time between two groups in the context of adjusting for baseline characteristics. Despite its theoretical nature, the phenomenon is deeply relevant to practical data analysis, especially in observational studies where causation is challenging to establish. Let’s look at an example.
...
- **Method 2** asks: “Given the same initial weight, do the groups end up at different final weights?”

The regression approach adjusts for baseline differences, assuming $W_{\text{pre}}$ is a confounder.

The key insight is that the choice of analysis reflects underlying assumptions about the causal structure of the data. If $W_{\text{pre}}$ is affected by group membership (e.g., due to selection bias), then adjusting for it may introduce bias rather than remove it. However, in practice we should usually control for baseline characteristics as they are critical in balancing the treatment and control groups.

### Simpson’s Paradox Once Again

I recently illustrated the more commonly discussed [Simpson’s paradox](https://vyasenov.github.io/blog/paradox-simpson.html). Interestingly, a [2008 paper](https://link.springer.com/article/10.1186/1742-7622-5-2) claims that the two phenomena are closely related, with Lord’s paradox being a “continuous version” of Simpson’s paradox. I have also written about [Stein’s paradox](https://vyasenov.github.io/blog/paradox-stein.html), another counterintuitive result in this vein.
...
Lord, F. M. (1975). Lord’s paradox. In S. B. Anderson, S. Ball, R. T. Murphy, & Associates, Encyclopedia of Educational Evaluation (pp. 232–236). San Francisco, CA: Jossey-Bass.
