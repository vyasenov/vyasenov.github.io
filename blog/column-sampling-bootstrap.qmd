...
The bootstrap is a versatile resampling technique traditionally focused on rows. Let’s add a twist to the [plain vanilla bootstrap](https://vyasenov.github.io/blog/flavors-bootstrap.html). Imagine you have a wide dataset—many variables but few rows—and want to test the statistical significance of a correlation between two variables. An example is a genetic dataset with thousands of columns (genetic information and outcomes) but a limited number of rows (patients). Can you use the bootstrap to determine if the correlation between a specific gene-outcome pair is statistically significant?
...
1. Randomly sample columns from your dataset with replacement to create a fake dataset.
...
The observed correlation is quite low and equal to $0.058$. Its associated $p$-value is $0.676$, consistent with the value not being statistically significant.


## Bottom Line

- The column-sampling bootstrap is a thought-provoking twist on [traditional resampling techniques](https://vyasenov.github.io/blog/jackknife-vs-bootstrap.html) that leverages the width of your dataset.
...
- The column-sampling bootstrap should not be your go-to method to assess statistical significance.