...
Stepwise selection can work well for smaller problems where computational cost is low and interpretability is key (although we have recently made some progress on the computation side).
...
Lasso is a good idea when you believe that only a subset of predictors are relevant and want an interpretable model.
