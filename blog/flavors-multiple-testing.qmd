...
If we are interested in measuring statistical significance, the standard frequentist framework relies on testing a single hypothesis on any given dataset. (I have written before about how this framework changes when we have [population rather than sample data](https://vyasenov.github.io/blog/hypothesis-testing-all-data.html).) Once we start using the same data multiple times – i.e., testing several hypotheses at once – as we often do, we might have to make some additional adjustments.
...
Vaguely speaking, MH adjustments can be viewed as inflating the calculated p-values and hence decreasing the probability that we reject any given hypothesis. In other words, we are accounting for using the data multiple times by making the definition of statistical significance more stringent. (For a different take on the p-value itself, I have written about [second-generation p-values](https://vyasenov.github.io/blog/second-gen-pvalues.html) elsewhere.)
...
This is much like the Bonferroni method, but with a multiplication factor of $(m+1-k)$ instead of $m$.
...
::: {.callout-note title="Algorithm:"}
1. Order the test statistics in descending order, $t_1 \geq t_2 \geq \dots \geq t_m$.
...
::: {.callout-note title="Algorithm:"}
1. Order the $p$-values in ascending order – $p_1, p_2, \dots, p_m$.
2. Find the largest integer $k$ such that $p_k \times \frac{m \cdot c(m)}{k}\leq \alpha$.
3. Reject the null hypotheses $H_1, \dots, H_k$.
:::
...
I will briefly mention some of the [newer methods that have caught my eye](https://vyasenov.github.io/blog/recent-dev-fdr.html) in recent months. Many of these control the FDR asymptotically, and some are valid under general dependency structures among the test statistics.
...
![Number of Statistically Significant Variables](../files/multiple-testing.png)
...
- In most settings, the Benjamini-Yekutieli approach offers a great balance between statistical power and technical simplicity.
