...
date: "2023-12-20"
categories: [machine learning, statistical inference]
---
...
Here $\hat{\mu}(\cdot)$ is an estimate of the regression function $E[Y \mid X]$, and $\hat{q}^{\mid \hat{u} \mid}_{1-\alpha}$ is the $(1-\alpha)$th quantile of the empirical distribution function of the fitted residuals $\mid Y-\hat{\mu}(X) \mid$.
...
where $I(\cdot)$ is the indicator function equal to one when the statement in the parenthesis is true and 0 when it is not.
...
1. Randomly split the data into two equal-sized bins.
...
The actual sepal length value in the test set was $6.2$ while the conformal inference approach computed a confidence interval ($5.89, 6.68$). The splitting algorithm gave similar results.
...
Conformal inference in machine learning is an ongoing research topic and I do not know of any review papers or textbook treatments of the subject. If you are interested in learning more, check the paper referenced below. In a follow-up article I also describe how to use [conformal inference for measuring variable importance](https://vyasenov.github.io/blog/conformal-inference-var-selection.html).
