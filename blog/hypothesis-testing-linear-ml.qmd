...
date: "2022-11-06"
categories: [statistical inference, machine learning, variable selection]
---
...
We are trying to predict a vector $Y\in \mathbb{R}$ with a set of features $X\in \mathbb{R}^{p \times n}$ with $p\leq n$, and $\lambda$ is a tuning parameter. When needed, I will use $j$ to index individual columns (i.e., variables) of $X$.
...
There is an alternative way to think about the problem. Imagine we run a variable selection algorithm (e.g., lasso), which selects a subset $M\subset \{1,\dots,p\}$ of all available predictors ($X$), $|M|<p$, leading to the alternative model:
...
Another and even more conservative choice for $m$ is the Scheffé constant.
...
In English, for fixed $X$ and $\lambda$, the set of alternative outcome values $Y^*$ which yields the same set of selected predictors, can be expressed by the simple inequality above. In it, $A$ and $b$ do not depend on $Y$. Under this new result, the distribution of $\hat{\beta}_M^{\text{EPoSI}}$ is now well-understood and tractable, thus enabling valid hypothesis testing.
...
Selective inference is currently among the hottest topics in all of statistics. There have been a myriad of extensions and improvements on the original paper. Still, this literature is painstakingly technical. What is the polyhedral selection property or a union of polytopes?
...
- If we believe we have to focus on the selected model, EPoSI is the state-of-the-art.
