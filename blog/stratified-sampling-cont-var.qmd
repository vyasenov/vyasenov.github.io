...
Commonly, $\text{distance}(X_i, \mu_j)=\|X_i - \mu_j\|^2$ which leads to $k$-means clustering. Unlike in the binning approach, here we are not restricting each stratum to have the same number of observations.

## Pros and Cons

Unsurprisingly, each method comes with its trade-offs. Traditional binning is simple and interpretable but can struggle with multivariate dependencies. Clustering accounts for multivariate relationships between variables, avoids imposing arbitrary bin thresholds, and may result in more natural groupings. However, it can be computationally expensive and sensitive to the choice of algorithm and hyperparameters (e.g., $k$ in $k$-means).
...
Here is `R` and `python` code illustrating both types of approaches on the popular `iris` dataset. We are interested in creating strata based on the `Sepal.Length` variable. We begin with the traditional binning approach.
