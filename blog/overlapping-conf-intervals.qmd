...
This is the interval we should be analyzing when testing whether $Y_1$ and $Y_2$ differ significantly. Gelman and Stern (2006) make the same point from a slightly different angle. (Here I assume the usual symmetric $\pm$ interval, though I have also written about [why some confidence intervals are not symmetric](https://vyasenov.github.io/blog/nonsymmetric-conf-int.html).)
...
This ratio is always greater than 1, meaning the naïve method uses a wider interval. It is more conservative when the null hypothesis is true (i.e., less likely to reject it), but less powerful when the null is false (i.e., more prone to false negatives).
...
$$
|\hat{Y}_1 - \hat{Y}_2| > 1.96 \times \sqrt{2} \times SE \approx 2.77 \times SE.
$$
