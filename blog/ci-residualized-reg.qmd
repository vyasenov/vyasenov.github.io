...
We assume that $X$ includes all relevant confounders, the linear model is correct, etc. so that this model has a [causal interpretation](https://vyasenov.github.io/blog/interpret-OLS-causal-inference.html). In econometrics jargon, this is a "structural" model.
...
Geometrically, we can think of the FWL theorem in terms of projections in vector space. The residuals $\tilde{D}$ and $\tilde{Y}$ are what remain after projecting $D$ and $Y$ onto the orthogonal complement of the space spanned by $X$.
