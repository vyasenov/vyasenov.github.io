...
date: "2024-10-29"
categories: [randomized experiments]
---
...
This mathematical convenience allows us to avoid more complex numerical methods like MCMC sampling. We can now even plot the two posterior distributions and visually assess their differences.
...
where $\mathbf{1}(\cdot)$ is the indicator function. This is simply the share of observations for which $Y_T >Y_C$.
...
- **Handling of Small Sample Sizes**: It tends to perform better with small sample sizes because Bayesian methods incorporate prior distributions, which can regularize estimates and prevent overfitting.
...
print(paste("Probability that T is better than C:", round(prob_T_better, digits=3)))

# Estimate the average treatment effect
treatment_effect <- mean(posterior_obs_T - posterior_obs_C)
print(paste("Average change in Y b/w T and C:", round(treatment_effect, digits=3)))
