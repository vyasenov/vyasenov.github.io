...
Randomized experiments are the gold standard when interested in measuring causal relationships with data. In settings with small treatment effects or underpowered designs, a major focus falls on decreasing the variance. In simple low-dimensional settings a common attempt to do that is to include a bunch of [covariates](https://vyasenov.github.io/blog/roles-covariates-rtc.html) and their interaction with the treatment variable in an OLS regression. Under [standard assumptions](https://projecteuclid.org/journals/annals-of-applied-statistics/volume-7/issue-1/Agnostic-notes-on-regression-adjustments-to-experimental-data--Reexamining/10.1214/12-AOAS583.full), the coefficient on the treatment variable is still asymptotically unbiased (albeit not in finite samples) and including the interactions guarantees that this estimator does not have higher asymptotic variance than the simple difference-in-means.
...
The simplest and most natural way to incorporate covariates is to add them to a linear model (along with the treatment variable and their interactions with the treatment variable). [Bloniarz et al. (2016)](https://www.pnas.org/doi/abs/10.1073/pnas.1510506113) show we can directly use Lasso regression instead of OLS.
...
1. For the treatment and control groups separately, run lasso regression of $Y$ on $\tilde{X}$ to get $\hat{\beta}^T_{lasso}$ and $\hat{\beta}^C_{lasso}$.
2. Calculate the treatment effect estimate $\hat{ATE}^{lasso}$ using the above formula.
3. Calculate the estimate of the variance of $\hat{ATE}^{lasso}$ using the formula in Bloniarz et al. (2016).
:::
