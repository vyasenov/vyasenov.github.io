...
In high-dimensional regression, we sometimes hear that a [variable selection](https://vyasenov.github.io/blog/flavors-var-selection.html) method possesses the oracle property. The phrase sounds impressive: it suggests that an estimator behaves as if the true sparsity pattern were known in advance—hence the name, as though an oracle had revealed the true support beforehand.
...
Put simply, $S$ is the set of variables that are non-zero in the true parameter vector $\beta$, and $s$ is the number of non-zero variables. 
...
::: {.callout-note title="Oracle Property Definition"}
Can a data-driven procedure simultaneously discover $S$ and then estimate as efficiently as if $S$ were given?
:::
...
Classical [LASSO](https://vyasenov.github.io/blog/flavors-lasso.html) does not generally satisfy the oracle property. Its $\ell_1$ penalty introduces shrinkage bias that persists asymptotically.
...
I have written about the [theoretical guarantees of LASSO](https://vyasenov.github.io/blog/lasso-theory-guarantees.html) and the design conditions they rely on before. In finite samples, especially when signals are weak or highly correlated, procedures that theoretically satisfy the oracle property may not outperform simpler methods. In practice, prediction risk often matters more than exact support recovery.
