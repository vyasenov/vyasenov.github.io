...
MI can be especially helpful for applications such as feature selection and unsupervised learning. For readers familiar with [various types of correlation metrics](https://vyasenov.github.io/blog/corr-myths-13-views.html), Mutual Information provides an additional lens to interpret relationships within data.
...
- **Independence**: Similarly to [Chatterjee’s correlation coefficient](https://vyasenov.github.io/blog/flavors-correlation-coef.html), $MI(X,Y) = 0$ if and only if $X$ and $Y$ are independent.
- **Scale-invariance**: Mutual information is scale invariant. If you apply a scaling transformation to the variables, their MI will not be affected.
...
For instance, methods like Minimum Redundancy Maximum Relevance (mRMR, Peng et al. 2005) aim to maximize the relevance of features to the target while minimizing redundancy among features. I've written about a related dependence-based approach in my post on [FOCI](https://vyasenov.github.io/blog/foci.html). Here is a concise version of the mRMR algorithm:
...
![](../files/mutual_info_overview.png)
...
[Wikipedia](https://en.wikipedia.org/wiki/Mutual_information) is a great place to start learning the basics. Brown et al. (2012) and Vergara and Estévez (2014) are the go-to resources for conditional MI and using MI for feature selection.
