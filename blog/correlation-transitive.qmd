...
At first, I found this really puzzling. $X$ is correlated (Pearson) with $Y$, and $Y$ is correlated with $Z$. Does this mean $X$ is necessarily correlated with $Z$? Intuitively, this totally makes sense. The answer, however, is “no.” This is just one of the many [myths and surprises](https://vyasenov.github.io/blog/corr-myths-13-views.html) I've explored around correlation.
...
$$cor(X,Y)cor(Z,Y) - \sqrt{(1-cor(X,Y)^2)(1-cor(Z,Y)^2)} \leq cor(X,Z) \leq cor(X,Y)cor(Z,Y) + \sqrt{(1-cor(X,Y)^2)(1-cor(Z,Y)^2)}.$$
