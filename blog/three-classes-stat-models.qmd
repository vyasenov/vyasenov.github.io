...
date: "2025-01-12"
categories: [parametric models, semiparametric models, nonparametric models]
---
...
Parametric models impose strong assumptions about the underlying data-generating process, making them less flexible but highly interpretable and data-efficient. I explore [their efficiency limits](https://vyasenov.github.io/blog/limits-parametric-models.html) in detail elsewhere. They require fewer data points to estimate the target accurately compared to semiparametric or nonparametric models. Consequently, they are often the default choice in practice.
...
Semiparametric models combine the structure of parametric models with the flexibility of nonparametric methods, and I dig into [their efficiency bound](https://vyasenov.github.io/blog/limits-semiparam-models.html) elsewhere. They specify certain aspects of the data-generating process parametrically while leaving other aspects unspecified or modeled flexibly.

Consider the partially linear model:

$$Y = X \beta + g(Z) + \epsilon,$$

where $\beta$ is a parametric component describing the linear effect of $X$, while $g(Z)$ is an unknown and potentially complex function capturing the nonparametric effect of a covariate matrix $Z$. [Generalized additive models](https://vyasenov.github.io/blog/generalized-additive-models.html) are a popular way to estimate such flexible components. Here, the model imposes linearity on $X$‘s effect but allows $Z$‘s effect to be fully flexible.
...
where $K(\cdot)$ is a kernel function, and $h$ is a bandwidth parameter controlling the smoothness of the estimate. A kernel function assigns varying weight to observations around a data point. They are non-negative, symmetric around zero, and sum to one. Commonly employed kernel functions include:
...
Did I also mention the curse of dimensionality?! You will quickly fall into its trap with even a moderate number of variables. Yes, there is just so much you can do without adding some structure to your models.
...
- Parametric models impose the strongest assumptions and require the least amount of data. These are the models most often employed in practice. Think of linear regression.

- Semiparametric models strike a balance between flexibility and interpretation while allowing for flexible relationships in the data. Think of (non-Augmented) Inverse Propensity Score Weighting.

- Nonparametric models are flexible and data-hungry. They allow for flexible associations between your variables. Think of a histogram or kernel density.
